...
export default function DrugDrugInteractionDetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchInteraction = async () => {
      try {
        ...
        setError("Failed to load drug-drug interaction details. Please try again later.")
        setLoading(false)
      }
    }
  ...
  const handleDelete = async () => {
    try {
      await drugDrugInteractionApi.delete(params.id)
    ...
    return (
      <div className="p-8">
        <div className="flex items-center gap-2 mb-6">
          <Link href="/admin/drug-drug-interactions" className="text-gray-600 hover:text-gray-900">
            <ArrowLeft className="h-5 w-5" />
          </Link>
          <h1 className="text-2xl font-bold">Drug-Drug Interaction Details</h1>
        </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Link href={`/admin/drug-drug-interactions/${params.id}/edit`} passHref>
          ...
          <CardContent>
            <div className="space-y-4">
              <div>
                ...
                <p>{interaction.medication1_name || `ID: ${interaction.medication1_id}`}</p>
              </div>
              <div>
                <h3 className="font-medium">Medication 2:</h3>
                <p>{interaction.medication2_name || `ID: ${interaction.medication2_id}`}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="space-y-2">
              <div className="flex justify-between">
                ...
                <span>{interaction.dd_interaction_id}</span>
              </div>
              <div className="flex justify-between">
