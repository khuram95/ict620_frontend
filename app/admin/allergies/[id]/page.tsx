...
export default function AllergyDetailPage({ params }: { params: { id: string } }) {
  ...
  const handleDelete = async () => {
    try {
      await allergyApi.delete(params.id)
      ...
      toast({
        title: "Error",
        description: "An error occurred while deleting the allergy.",
        variant: "destructive",
      })
    }
  }
    ...
    return (
      <div className="p-8">
        <div className="flex items-center gap-2 mb-6">
          <Link href="/admin/allergies" className="text-gray-600 hover:text-gray-900">
            <ArrowLeft className="h-5 w-5" />
          </Link>
          <h1 className="text-2xl font-bold">Allergy Details</h1>
        </div>
    ...
    return (
      <div className="p-8">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h1 className="text-2xl font-bold">Allergy Not Found</h1>
        </div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold">{allergy.name}</h1>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/admin/allergies/${params.id}/edit`} passHref>
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              <div>
                ...
                <p>{allergy.description || "No description provided"}</p>
              </div>
              <div>
          ...
          <CardContent>
            <div className="space-y-2">
              <div className="flex justify-between">
                ...
                <span>{allergy.allergy_id}</span>
              </div>
              <div className="flex justify-between">
                <span className="font-medium">Created:</span>
                <span>{new Date(allergy.created_at).toLocaleString()}</span>
              </div>
              <div className="flex justify-between">
                <span className="font-medium">Last Updated:</span>
                <span>{allergy.updated_at ? new Date(allergy.updated_at).toLocaleString() : "Never"}</span>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete the allergy.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
