...
export default function DrugFoodInteractionsPage() {
  ...
  useEffect(() => {
    const fetchInteractions = async () => {
      try {
        setLoading(true)
        const data = await drugFoodInteractionApi.getAll()
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      // await drugFoodInteractionApi.delete(deleteId)
      setInteractions(interactions.filter((interaction) => interaction.df_interaction_id.toString() !== deleteId))
      toast({
      ...
      render: (item: DrugFoodInteraction) => (
        <div>
          ...
          <div className="flex items-center gap-1">
            ...
            <span>{item.food_name || `Food ID: ${item.food_id}`}</span>
          </div>
        </div>
      ),
    },
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable
        data={interactions}
        columns={columns}
        primaryKey="df_interaction_id"
        basePath="/admin/drug-food-interactions"
